...
{
    /// <summary>
    /// Построитель виртуальных иерархий по типам товара 
    /// </summary>
    public class VirtualHierarchyProductKinds : IVirtualGroupBuilder<Product>
    {
        /// <summary>
        ...
        public List<IVirtualGroup<Product>> Roots()
        {
            if (_root == null)
            {
                ...
                //
                foreach (EntityKind c in _hierarchy.Workarea.CollectionEntityKinds.Where(f=>f.EntityId ==1))
                {
                    Hierarchy h = new Hierarchy {Workarea = _hierarchy.Workarea, IsVirtual = true, Name = c.ToString(), KindValue = 2, Code = c.Id.ToString()};
        ...
        void ValContentRequest(IVirtualGroup<Product> obj)
        {
            ...
            List<Product> coll = _allData.Where(s => s.KindId == id).ToList();
