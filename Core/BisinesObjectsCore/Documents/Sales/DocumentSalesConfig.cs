...
{
    /// <summary>
    /// Специальный документ настройки раздела "Управление продажами"
    /// </summary>
    public class DocumentSalesConfig:DocumentSales
    {
        
        /// <summary>
        /// Конструктор
        /// </summary>
        public DocumentSalesConfig():base()
        {
            /*
             currentConfig = new ConfigSales();
                        currentConfig.Reset();
             */
        }
        /// <summary>
        ...
        /// <returns></returns>
        public static DocumentSalesConfig CreateCopy(DocumentSalesConfig value)
        {

            DocumentSalesConfig salesDoc = new DocumentSalesConfig { Workarea = value.Workarea };
        ...
        private ConfigSales _config;
        /// <summary>
        /// Настройки раздела
        /// </summary>
        public ConfigSales Config
        {
            get
            {
                if (_config == null)
                {
                    _config = new ConfigSales();
                    _config.Reset();
                    if (Id != 0)
                    {
                        _config = new ConfigSales();
                        ...
                        {
                            _config = ConfigSales.Load(coll[0].Xml);
                        }
                    }
                }
                return _config;
