...
{
    /// <summary>
    /// Интерфейсный модуль "Устройства"
    /// </summary>
    public sealed class ContentModuleDevice : ContentModuleBase<Device>
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        ...
        /// отображения модуля.
        /// Заголовок модуля по умолчанию - "Устройства".
        ...
        /// <remarks>После отображения свойств объекта он добавляется в текущий список.</remarks>
        /// <param name="value">Объект для отображения</param>
        void OnShowPropTreeList(Device value)
        {
            value.ShowProperty();
            if (value.IsNew)
            {
        ...
        void OnSaveObject(Device value)
        {
            value.Save();
        }
        /// <summary>
        ...
        void ContentModuleShow(object sender, EventArgs e)
        {
            if (BrowserBaseObjects != null)
            {
                if (_showProp == null)
                {
                    _showProp = new Action<Device>(OnShowProp);
            ...
            if (TreeListBrowser != null)
            {
                ...
                {
                    _showPropTreeList = new Action<Device>(OnShowPropTreeList);
                    TreeListBrowser.ListBrowserBaseObjects.ShowProperty += _showPropTreeList;
                }
                if (_saveDevice == null)
                {
                    _saveDevice = new Action<Device>(OnSaveObject);
                    TreeListBrowser.ListBrowserBaseObjects.Save += _saveDevice;
                }
            }
        }
