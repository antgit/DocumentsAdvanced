...
{
    /// <summary>
    /// Документ "Прайс-лист поставщика" в разделе "Ценовая плолитика"
    /// </summary>
    /// <remarks>Документ для учета цен и выполнения расчетов цен.</remarks>
    public sealed class DocumentViewPriceListSuplyer : BaseDocumentViewPrice<DocumentPrices>, IDocumentView
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        public DocumentViewPriceListSuplyer()
            : base()
        {
        ...
        {
            DocumentViewPriceListSuplyer newDoc = new DocumentViewPriceListSuplyer();
        ...
        void CmbAgentFromButtonClick(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            //if (e.Button.Index == 0) return;
            //List<Agent> coll = Workarea.Empty<Agent>().BrowseListType(null, Workarea.GetCollection<Agent>(4));
            //if (coll == null) return;
        ...
        void CmbAgentGridLookUpEditQueryPopUp(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    CollectionPrice = Workarea.GetCollection<PriceName>(PriceName.KINDVALUE_PROVIDER).Where(s=>!s.IsHiden && s.IsStateAllow).ToList();
        ...
        // Обработка отрисовки изображения списке корреспондента "Кому"
