...
using BusinessObjects.DocumentLibrary.Sales;
...
namespace BusinessObjects.DocumentLibrary.Tax
{
    /// <summary>
    ///  Исходящая корректирвочная налоговая накладная
    /// </summary>
    public sealed class DocumentViewTaxCorOut : BaseDocumentViewTaxes<DocumentTaxes>, IDocumentView
    {
        ...
        {
            DocumentViewTaxCorOut newDoc = new DocumentViewTaxCorOut();
            newDoc.Showing += delegate
                                  {
                                      ...
                                      foreach (DocumentDetailTax item in from prodItem in SourceDocument.Details
                                                                         where prodItem.StateId == State.STATEACTIVE
                                                                         select new DocumentDetailTax
                                                                                    {
                                                                                        Workarea = Workarea,
        ...
        public override void BuildPageCommon()
        {
            if (ControlMain == null)
            {
                if (ControlMain != null)
                    return;
                ControlMain = new ControlTaxes { Name = ExtentionString.CONTROL_COMMON_NAME };
                ...
                if (SourceDocument == null)
                    SourceDocument = new DocumentTaxes { Workarea = Workarea };
                if (Id != 0)
                {
                ...
                else
                {
                    ...
                    if (DocumentTemplateId != 0)
                    {
                        ...
                        DocumentTaxes docHeaderDetailTemplate = Workarea.Cashe.GetCasheData<DocumentTaxes>().Item(DocumentTemplateId);
                        if (docHeaderDetailTemplate != null)
                        {
                            SourceDocument.DeliveryCondition = docHeaderDetailTemplate.DeliveryCondition;
                            SourceDocument.PaymentMethod = docHeaderDetailTemplate.PaymentMethod;
                        }
                ...
                ControlMain.cmbDeliveryCondition.Text = SourceDocument.DeliveryCondition;
                ControlMain.cmbPaymentMethod.Text = SourceDocument.PaymentMethod;
                ControlMain.cmbPaymentMethod.Properties.Items.AddRange(new[]{
                                                                                "С расчетного счета",
                                                                                "Наличный расчет"
                                                                            });
                ControlMain.cmbDeliveryCondition.Properties.Items.AddRange(new[]{
                                                                                    "Согласно накладной",
                                                                                    "В соответствии с договором"
                                                                                });
                ControlMain.txtDogovorNo.Text = SourceDocument.DogovorNo;
                ControlMain.dtDogovorDate.EditValue = SourceDocument.DogovorDate;
                ...
                BindSourceDetails.AddingNew += delegate(object sender, System.ComponentModel.AddingNewEventArgs eNew)
                                                   {
                                                       ...
                                                       {
                                                           eNew.NewObject = new DocumentDetailTax { Workarea = Workarea, Document = SourceDocument, StateId = State.STATEACTIVE };
                                                       }
            ...
            PrintDataDocumentHeader prnDoc = new PrintDataDocumentHeader
                                                 {
                                                     ...
                                                     PaymentMethod = SourceDocument.PaymentMethod,
                                                     DeliveryCondition = SourceDocument.DeliveryCondition,
                                                     DogovorNo = SourceDocument.DogovorNo,
                                                     DogovorDate = SourceDocument.DogovorDate
                                                 };
            if (SourceDocument.Document.AgentDepartmentFromId != 0)
            {
                prnDoc.AgFromName = string.IsNullOrEmpty(SourceDocument.Document.AgentDepartmentFrom.NameFull)
                                        ? SourceDocument.Document.AgentDepartmentFrom.Name
                ...
                prnDoc.AgentFromInn = SourceDocument.Document.AgentDepartmentFrom.CodeTax;
                prnDoc.AgentFromReg = SourceDocument.Document.AgentDepartmentFrom.Company.RegNumber;
                prnDoc.AgentFromPhone = SourceDocument.Document.AgentDepartmentFrom.Phone;
            }
            if (SourceDocument.Document.AgentToId != 0)
            {
                prnDoc.AgToName = string.IsNullOrEmpty(SourceDocument.Document.AgentTo.NameFull)
                                      ? SourceDocument.Document.AgentToName
                                      : SourceDocument.Document.AgentTo.NameFull;
                prnDoc.AgentToAddres = SourceDocument.Document.AgentTo.AddressLegal;
                prnDoc.AgentToPhone = SourceDocument.Document.AgentTo.Phone;

                if (SourceDocument.Document.AgentTo.Company.NdsPayer)
                {
                    prnDoc.AgentToInn = SourceDocument.Document.AgentTo.CodeTax;
                    prnDoc.AgentToReg = SourceDocument.Document.AgentTo.Company.RegNumber;
                }
                else
                {
                    prnDoc.AgentToInn = "ХХХХХХХХХХХХ";
                    prnDoc.AgentToReg = "XXXXXXXX";
                }
            }
        ...
        public override bool InvokeSave()
        {
            ...
            SourceDocument.DeliveryCondition = ControlMain.cmbDeliveryCondition.Text;
            SourceDocument.PaymentMethod = ControlMain.cmbPaymentMethod.Text;
            SourceDocument.DogovorNo = ControlMain.txtDogovorNo.Text;
            if (ControlMain.dtDogovorDate.EditValue == null)
                SourceDocument.DogovorDate = null;
            else
                SourceDocument.DogovorDate = ControlMain.dtDogovorDate.DateTime;
            ...
            try
            {
                ...
                if (SourceDocument.SalesDocId != 0)
                {
                    DocumentSales sd = Workarea.Cashe.GetCasheData<DocumentSales>().Item(SourceDocument.SalesDocId);
                    if (sd != null && sd.TaxDocId != SourceDocument.Id)
                    {
                        sd.TaxDocId = SourceDocument.Id;
                        sd.Save();
                        if (SourceView != null && (SourceView is DocumentViewSaleOut))
                            (SourceView as DocumentViewSaleOut).SourceDocument.Refresh(false);
                    }
                }
                return true;
            }
        ...
        void ViewDetailValidatingEditor(object sender, DevExpress.XtraEditors.Controls.BaseContainerValidateEditorEventArgs eEd)
        {
            if (ControlMain.ViewDetail.FocusedColumn.Name == "gridColumnName" | ControlMain.ViewDetail.FocusedColumn.Name == "gridColumnNom")
            {
                ...
                if (ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax != null &&
                    eEd.Value != null)
                {
                    ...
                    if (prod != null)
                    {
                        (ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax).Unit = prod.Unit;
                    }
                }
            }
            if (ControlMain.ViewDetail.FocusedColumn.Name == "gridColumnQty")
            {
                    ...
                    eEd.Value != null)
                {
                    ...
                    (ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax).Summa = val * (ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax).Price;
            ...
            if (ControlMain.ViewDetail.FocusedColumn.Name == "gridColumnPrice")
            {
                ...
                {
                    decimal val = Convert.ToDecimal(eEd.Value);
                    (ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax).Summa = val * (ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax).Qty;
            ...
            if (ControlMain.ViewDetail.FocusedColumn.Name == "gridColumnSumm")
            {
                ...
                {
                    decimal val = Convert.ToDecimal(eEd.Value);
                    if ((ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax).Qty != 0)
                        (ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax).Price = val / (ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax).Qty;
        ...
        {
            if ((BindSourceDetails.List[e.ListSourceRow] as DocumentDetailTax).StateId == 5)
            {
                e.Visible = false;
                e.Handled = true;
            }
        ...
        void EditNomProcessNewValue(object sender, DevExpress.XtraEditors.Controls.ProcessNewValueEventArgs eNv)
        {
            ...
            {
                int index = ControlMain.ViewDetail.FocusedRowHandle;
                DocumentDetailTax docRow = ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax;
            ...
            {
                int index = ControlMain.ViewDetail.FocusedRowHandle;
                if ((ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax).Product != null)
                    (ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax).Unit =
                        (ControlMain.ViewDetail.GetRow(index) as DocumentDetailTax).Product.Unit;
            }
        }
        ...
        void CmbAgentGridLookUpEditQueryPopUp(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            try
            {
                ...
                //{
                //    CollectionAgentDelivery = Agent.GetChainSourceList(Workarea, (int)ControlMain.cmbAgentDepatmentFrom.EditValue, DocumentViewConfig.DepatmentChainId);
                //    if ((int)ControlMain.cmbAgentFrom.EditValue == _sourceDocument.Document.AgentFromId &&
                //        (int)ControlMain.cmbAgentDepatmentFrom.EditValue == _sourceDocument.Document.AgentDepatmentFromId)
                //    {
                ...
                //{
                //    CollectionStore = Agent.GetChainSourceList(Workarea, (int)ControlMain.cmbAgentDepatmentFrom.EditValue, DocumentViewConfig.StoreChainId);
