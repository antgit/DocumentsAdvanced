...
namespace BusinessObjects.Windows
{
    ...
    /// </summary>
    internal sealed class BuildControlTerritory : BasePropertyControlIBase<Territory>
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        public BuildControlTerritory()
            : base()
        {
            TotalPages.Add(ExtentionString.CONTROL_COMMON_NAME, ExtentionString.CONTROL_COMMON_NAME);
            TotalPages.Add(ExtentionString.CONTROL_REGIONS, ExtentionString.CONTROL_REGIONS);
            TotalPages.Add(ExtentionString.CONTROL_TOWNS, ExtentionString.CONTROL_TOWNS);
            TotalPages.Add(ExtentionString.CONTROL_LINKTOWNS, ExtentionString.CONTROL_LINKTOWNS);
        ...
        protected override void BuildPage(string value)
        {
            ...
            if (value == ExtentionString.CONTROL_TOWNS)
                BuildPageTowns();
            if (value == ExtentionString.CONTROL_LINKTOWNS)
                BuildPageLinkTowns();
            if (value == ExtentionString.CONTROL_REGIONS)
                BuildPageRegions();
            if (value == ExtentionString.CONTROL_LINKFILES)
                BuildPageLinkedFiles();
        }
        
        ControlList _controlTowns;
        // Связянные города на основании принадлежности города к области
        private void BuildPageTowns()
        {
            if (_controlTowns == null)
            {
                _controlTowns = new ControlList {Name = ExtentionString.CONTROL_TOWNS};
                // Данные для отображения в списке связей
                BindingSource collectionBind = new BindingSource();
                List<Town> collection = SelectedItem.Workarea.GetCollection<Town>().Where(s => s.TerritoryId == SelectedItem.Id).ToList();
                collectionBind.DataSource = collection;
                _controlTowns.Grid.DoubleClick += delegate
                {
                    if (collectionBind.Current != null)
                        (collectionBind.Current as Town).ShowProperty();
                };
                ...
                BarButtonItem btn = new BarButtonItem {Caption = "Город"};
                mnuTemplates.AddItem(btn);
                btn.ItemClick += delegate
                {
                    Town newTown = new Town { Workarea = SelectedItem.Workarea, TerritoryId = SelectedItem.Id };
                    newTown.ShowProperty().FormClosed += delegate(object s, FormClosedEventArgs ev)
                    {
                        if (s != null)
                        {
                            Form f = s as Form;
                            ...
                            {
                                int index = collectionBind.Add(newTown);
                ...
                {
                    Town currentTown = collectionBind.Current as Town;
                    if (currentTown != null)
                    {
                        if (MessageBox.Show("Вы уверенны, что хотите удалить указанный объект?", "Внимание", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                        {
                            ...
                            {
                                currentTown.Delete();
                                collectionBind.Remove(currentTown);
                            }
                ...
                DataGridViewHelper.GenerateGridColumns(SelectedItem.Workarea, _controlTowns.View, "DEFAULT_LISTVIEW");
                _controlTowns.View.CustomDrawCell += delegate(object sender, DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs e)
                {
                ...
                Control.Controls.Add(_controlTowns);
                _controlTowns.Dock = DockStyle.Fill;

                _controlTowns.Grid.DataSource = collectionBind;
            }
        ...
        private void BuildPageLinkTowns()
        {
            ...
            {
                _controlLinkTowns = new ControlList { Name = ExtentionString.CONTROL_LINKTOWNS };
                ...
                RibbonPage page = frmProp.ribbon.Pages[ExtentionString.GetPageNameByKey(SelectedItem.Workarea, ExtentionString.CONTROL_LINKTOWNS)];
                RibbonPageGroup groupLinksAction = new RibbonPageGroup();
            ...
            HidePageControls(ExtentionString.CONTROL_LINKTOWNS);
        }
        #endregion

        ControlList _controlRegions;
        private void BuildPageRegions()
        {
            if (_controlRegions == null)
            {
                _controlRegions = new ControlList {Name = ExtentionString.CONTROL_REGIONS};
                // Данные для отображения в списке связей
                BindingSource collectionBind = new BindingSource();
                int chainId = SelectedItem.Workarea.CollectionChainKinds.Find(
                    s =>
                    s.Code == ChainKind.TREE && s.FromEntityId == SelectedItem.EntityId &&
                    s.ToEntityId == SelectedItem.EntityId).Id;
                List<IChain<Territory>> collection = SelectedItem.GetLinks(chainId);
                collectionBind.DataSource = collection;
                // Построение группы упраления связями
                RibbonPage page = frmProp.ribbon.Pages[ExtentionString.GetPageNameByKey(SelectedItem.Workarea, ExtentionString.CONTROL_REGIONS)];
                RibbonPageGroup groupLinksAction = new RibbonPageGroup();
                _controlRegions.Grid.DoubleClick += delegate
                {
                    if (collectionBind.Current != null)
                    {
                        (collectionBind.Current as IChain<Territory>).Right.ShowPropertyRegion();
                    }
                };
                ...
                PopupMenu mnuTemplates = new PopupMenu {Ribbon = frmProp.ribbon};
                BarButtonItem btn = new BarButtonItem {Caption = "Областной район"};
                mnuTemplates.AddItem(btn);
                btn.ItemClick += delegate
                {
                    Territory newRegion = new Territory { Workarea = SelectedItem.Workarea, CountryId = SelectedItem.CountryId };
                    newRegion.ShowPropertyRegion().FormClosed += delegate(object s, FormClosedEventArgs ev)
                    {
                        if (s != null)
                        {
                            Form f = s as Form;
                            if (f.DialogResult == DialogResult.OK)
                            {
                                Chain<Territory> newChain = new Chain<Territory>(SelectedItem)
                                                                {
                                ...
                                newChain.Save();
                                int index = collectionBind.Add(newChain);
                                collectionBind.Position = index;
                            }
                        }
                    };
                ...
                btnChainDelete.ItemClick += delegate
                {
                    Chain<Territory> currentObject = collectionBind.Current as Chain<Territory>;
                    if (currentObject != null)
                    {
                        int res = Extentions.ShowMessageChoice(SelectedItem.Workarea,
                            SelectedItem.Workarea.Cashe.ResourceString(ResourceString.MSG_CAPATTENTION, 1049), "Удаление связей",
                ...
                page.Groups.Add(groupLinksAction);
                DataGridViewHelper.GenerateGridColumns(SelectedItem.Workarea, _controlRegions.View, "DEFAULT_LISTVIEWCHAIN");
                for (int i = 0; i < _controlRegions.View.Columns.Count; i++)
                    _controlRegions.View.Columns[i].GroupIndex = -1;
                _controlRegions.View.CustomDrawCell += delegate(object sender, DevExpress.XtraGrid.Views.Base.RowCellCustomDrawEventArgs e)
                {
                    if (e.Column.Name == "colImage")
                    {
                        Rectangle r = e.Bounds;
                ...
                Control.Controls.Add(_controlRegions);
                _controlRegions.Dock = DockStyle.Fill;

                _controlRegions.Grid.DataSource = collectionBind;
            }
            HidePageControls(ExtentionString.CONTROL_REGIONS);
        }
