...
{
    /// <summary>
    /// Представление списка документов договоров для Web интерфейса
    /// </summary>
    public sealed class ContractDocumentsWebView
    {
        static ContractDocumentsWebView()
        {
        ...
        /// <param name="ds">Начало пероида</param>
        /// <param name="de">Конец периода</param>
        /// <param name="count">Число возвращаемых записей</param>
        /// <param name="stateId">Ид состояния</param>
        /// <param name="refresh">Обновить данные из БД</param>
        /// <returns></returns>
        public static DataTable GetView(Workarea wa, int kindId, string folderCodeFind, string userName, DateTime ds, DateTime de, int? stateId, int? count = null, bool refresh = false)
        {
            ...
            string keyData = string.Format("{0}{1}{2}{3}{4}{5}{6}", kindId, folderCodeFind, userName, ds, de, stateId, count);
            ...
            using (SqlConnection cnn = wa.GetDatabaseConnection())
            {
                ...
                try
                {
                    ...
                    {
                        string procedureName = "[Contracts].[DocumentsLoadByKind]";
