...
namespace BusinessObjects.Windows
{
    public static partial class Extentions
    {
        ...
        /// <returns></returns>
        public static Form ShowProperty(this Device item)
        {
            InternalShowPropertyBase<Device> showPropertyBase = new InternalShowPropertyBase<Device>();
            showPropertyBase.SelectedItem = item;
            showPropertyBase.ControlBuilder = new BuildControlDevice { SelectedItem = item };
        ...
        public static Device BrowseList(this Device item)
        {
            List<Device> col = BrowseMultyList(item, item.Workarea, null, null, false);
        ...
        /// <returns></returns>
        public static List<Device> BrowseList(this Device item, Predicate<Device> filter, List<Device> sourceCollection, string rootCode = null)
        {
            return BrowseMultyList(item, item.Workarea, filter, sourceCollection, true, rootCode);
        }
        internal static List<Device> BrowseMultyList(this Device item, Workarea wa, Predicate<Device> filter, List<Device> sourceCollection, bool allowMultySelect, string rootCode = null)
        {
            List<Device> returnValue = null;
            FormProperties frm = new FormProperties();
            Bitmap img = ExtentionsImage.GetImage(wa.Empty<Device>().Entity);
            frm.Ribbon.ApplicationIcon = img;
            frm.Icon = Icon.FromHandle(img.GetHicon());

            ListBrowserBaseObjects<Device> browserBaseObjects = new ListBrowserBaseObjects<Device>(wa, sourceCollection, filter, item, true, false, false, true) { Owner = frm, RootCode = rootCode };
            ...
            browserBaseObjects.ShowProperty += delegate(Device obj)
            {
                obj.ShowProperty();
                if (obj.IsNew)
                {
        ...
        public static List<Device> BrowseContent(this Device item, Workarea wa = null)
        {
            ContentModuleDevice module = new ContentModuleDevice();
