...
namespace BusinessObjects.DocumentLibrary.Person
{
    ...
    /// </summary>
    public sealed class DocumentViewPersonOut : BaseDocumentViewPerson<DocumentPerson>, IDocumentView
    {
        #region Создание копии и нового документа
        protected override void CreateCopy()
        {
            DocumentViewPersonOut newDoc = new DocumentViewPersonOut();
        ...
        public override void BuildPageCommon()
        {
            if (ControlMain == null)
            {
                ...
                {
                    CollectionDepatmentTo.Add(Workarea.Cashe.GetCasheData<Depatment>().Item(personXml.DepatmentFromId));
                }
                BindSourceDepatmentTo = new BindingSource { DataSource = CollectionDepatmentTo };
                ControlMain.cmbDepatmentFrom.Properties.DataSource = BindSourceDepatmentTo;
                DataGridViewHelper.GenerateGridColumns(Workarea, ControlMain.ViewDepatmentToXml, "DEFAULT_LOOKUP_NAME");
                if (personXml != null)
                ...
                ControlMain.layoutControlItemDepatmentTo.Visibility = DevExpress.XtraLayout.Utils.LayoutVisibility.Never;
