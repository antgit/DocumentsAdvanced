...
DECLARE @Kind INT=131074 -- Тип операции
DECLARE @FormId INT=74   -- Ид формы документа
DECLARE @FolderId INT=54   -- Ид папки
...
INSERT @tblTmlId(Id)
SELECT TML_ID FROM Донецк7.dbo.TEMPLATES t WHERE 
t.TML_NAME IN('11.04.01. Приход товаров из центра')
...
SELECT d.DOC_ID AS Id, d.DOC_GUID AS Guid, d.DOC_DATE, d.DOC_NAME AS Name, d.DOC_NO, @Kind AS Kind,@FormId AS FormId, @FolderId AS FolderId,
  ...
  LEFT JOIN Донецк7.dbo.AGENTS a2 ON j.J_AG2 = a2.AG_ID
...
SELECT d.Guid,3,d.SourceId,0,1,d.NAME,d.KindId,d.Date, d.FormId, d.FolderId,
       a2.Id,           -- Предприятие Кто
       null,           -- Подразделение Кто
       @RootCompanyId, -- Предприятие кому
       @MyCompanyId,   -- Подразделение кому
       --
       a2.[Name],'ЧП "Украинский продукт"',515,d.Summa,d.Number
  FROM 
