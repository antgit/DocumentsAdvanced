...
{
    internal class GeneratorProcedureExistsGuid : DataObjectGenerator, IDataObjectGenerator2
    {
        public GeneratorProcedureExistsGuid()
        {
            /*
            ...
            Template = "SET ANSI_NULLS ON \n"
                       ...
                       + "@Values dbo.KeyListGuid READONLY \n"
                       + "AS \n"
                       + "set nocount on \n"
                       + "BEGIN TRY \n"
                       + "SELECT v.Guid, CASE WHEN a.Id IS NOT NULL THEN a.Id ELSE 0 END AS Id \n"
                       + "      FROM @Values v LEFT JOIN %schema%.%table% a ON v.Guid = a.Guid \n"
                       + "RETURN 0; \n"
                       + "END TRY \n"
                       + "BEGIN CATCH \n"
        ...
        public override string Generate()
        {
            ...
            string procname = "[" + Schema + "].[" + TableName + "ExistsGuids" + "]";
