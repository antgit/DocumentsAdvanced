...
{
    /// <summary>
    /// Формирование контрола для отображения свойств оборудования
    /// </summary>
    internal sealed class BuildControlEquipment : BasePropertyControlIBase<Equipment>
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        public BuildControlEquipment()
            : base()
        {
        ...
        ControlEquipment _common;
        protected override void BuildPageCommon()
        {
            if (_common == null)
            {
                _common = new ControlEquipment
                              {
                ...
                UIHelper.GenerateTooltips<Equipment>(SelectedItem, _common);
        ...
        private List<IChainAdvanced<Equipment, FileData>> _collectionFiles;
        ...
        void ViewCustomUnboundColumnDataFiles(object sender, CustomColumnDataEventArgs e)
        {
            ...
            {
                ChainAdvanced<Equipment, FileData> link = _bindFiles[e.ListSourceRowIndex] as ChainAdvanced<Equipment, FileData>;
        ...
        protected void InvokeFileDelete()
        {
            ...
            ChainAdvanced<Equipment, FileData> link = _bindFiles.Current as ChainAdvanced<Equipment, FileData>;
            if (link == null) return;
        ...
        protected void InvokeFileLink()
        {
            ...
            foreach (ChainAdvanced<Equipment, FileData> link in
                retColl.Select(item => new ChainAdvanced<Equipment, FileData>(SelectedItem) { Right = item, StateId = State.STATEACTIVE, KindId = ckind.Id }))
            {
                link.Save();
        ...
        protected void InvokeFileCreate()
        {
            ...
            foreach (string fileName in dlg.FileNames)
            {
                ...
                ChainAdvanced<Equipment, FileData> link =
                    new ChainAdvanced<Equipment, FileData>(SelectedItem) { Right = fileData, StateId = State.STATEACTIVE, KindId = ckind.Id };
        ...
        List<FilePreviewThread<Equipment>> OpennedDocs;
        protected void InvokeFilePreview()
        {
            try
            {
                ...
                if (_bindFiles.Current == null) return;
                if (OpennedDocs == null)
                    OpennedDocs = new List<FilePreviewThread<Equipment>>();
                for (int i = OpennedDocs.Count - 1; i >= 0; i--)
                    if (OpennedDocs[i].IsExit)
                        OpennedDocs.Remove(OpennedDocs[i]);
                FilePreviewThread<Equipment> dpt = new FilePreviewThread<Equipment>(_bindFiles.Current as ChainAdvanced<Equipment, FileData>) { OpennedDocs = OpennedDocs }; //{ DocView = this };
                OpennedDocs.Add(dpt);
            }
            catch (Exception ex)
            {
