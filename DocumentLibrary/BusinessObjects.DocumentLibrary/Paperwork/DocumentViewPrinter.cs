...
{
    /// <summary>
    /// Документ "Учет принтеров" в разделе "Управление договорами"
    /// </summary>
    public class DocumentViewPrinter : BaseDocumentView<DocumentContract>, IDocumentView //BaseDocumentViewContract<DocumentContract>, IDocumentView
    {
        #region Базовые функции
        // элемент отображения
        internal ControlContractPrinter ControlMain;
        ...
        {
            DocumentViewPrinter newDoc = new DocumentViewPrinter();
        ...
        public override void BuildPageCommon()
        {
            if (ControlMain == null)
            {
                if (ControlMain != null)
                    return;
                ControlMain = new ControlContractPrinter { Name = ExtentionString.CONTROL_COMMON_NAME };
                ...
                if (MainDocument.IsReadOnly)
                {
                    ...
                    else
                    {
                        ...
                        collProducts = Workarea.GetCollection<Product>(Product.KINDVALUE_ORGTEX);
                    }
                }
                else
                    collProducts = Workarea.GetCollection<Product>(Product.KINDVALUE_ORGTEX);
