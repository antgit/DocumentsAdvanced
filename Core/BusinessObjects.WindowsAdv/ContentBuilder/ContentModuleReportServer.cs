...
namespace BusinessObjects.Windows
{
    ...
    /// </summary>
    public class ReportServerContentModule : ContentModuleBase<Library>
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        ...
        /// отображения модуля.
        /// Заголовок модуля по умолчанию - "Сервер отчетов".
        ...
        /// </remarks>
        public ReportServerContentModule()
        {
            Caption = "Сервер отчетов";
            Show += ContentModuleShow;
            CreateControlTreeList += ContentModuleCreateTreeList;
            TYPENAME = "REPORTSERVER";
            Key = TYPENAME + "_MODULE";
            RootCode = "REPORTSMODULEDATA";
            ShowProperiesOnDoudleClick = false;
            RestrictedTemplateKinds.Add(Library.KINDVALUE_REPSQL);
        }
        private void OnRefreshFromServer()
        {
            try
            {
                if (_bindServers.Current == null)
                    return;
                if (!TreeListBrowser.TreeBrowser.SelectedTreeIsHierarchy)
                    return;
                ...
                BusinessObjects.ReportingService.Reports reps = new ReportingService.Reports();
                //folderName: TreeListBrowser.TreeBrowser.SelectedHierarchy.Name
                string rootFolder = "/Документы2011/" + TreeListBrowser.TreeBrowser.SelectedHierarchy.Name;
                reps.FillFromSourceServer(srv, TreeListBrowser.TreeBrowser.SelectedHierarchy, rootFolder);
            }
            catch(Exception ex)
            {
                Extentions.ShowMessagesExeption(this.Workarea,
                                                Workarea.Cashe.ResourceString(ResourceString.MSG_CAPERROR,
                                                                                           1049), string.Empty, ex);
            }
        }
        private void OnEditFromServer()
        {
            try
            {
                if (_bindServers.Current == null)
                    return;
                string srv = (_bindServers.Current as Analitic).Code;
                Library item = TreeListBrowser.ListBrowserBaseObjects.FirstSelectedValue;
                if (item == null) return;
                item.EditSqlReport(srv);
            }
            catch (Exception ex)
            {
                Extentions.ShowMessagesExeption(this.Workarea,
        ...
        {
            Image32 = ResourceImage.GetByCode(Workarea, ResourceImage.REPORT_X32);
        }
        /// <summary>
        ...
        void ContentModuleCreateTreeList(object sender, EventArgs e)
        {
            ...
            #region Кнопка "Обновить с сервера отчетов"
            if (SecureLibrary.IsAllow(UserRightElement.UIREPORTLISTFILL, SelfLibrary.Id) || Workarea.Access.RightCommon.AdminEnterprize)
            {
                BarButtonItem btnFillFromServer = new BarButtonItem
                {
                    Caption = Workarea.Cashe.ResourceString(ResourceString.BTN_CAPTION_REFRESHRS, 1049),
                    //"Обновить с сервера отчетов",
                    RibbonStyle = RibbonItemStyles.SmallWithText,
                    Glyph = ResourceImage.GetByCode(Workarea, ResourceImage.REFRESHGREEN_X16)
                };
                btnFillFromServer.SuperTip = UIHelper.CreateSuperToolTip(btnFillFromServer.Glyph, Workarea.Cashe.ResourceString(ResourceString.BTN_CAPTION_REFRESHRS, 1049),
                    "Обновить с сервера отчетов");

                groupLinksActionTreeList.ItemLinks.Insert(groupLinksActionTreeList.ItemLinks[0], btnFillFromServer);
                btnFillFromServer.ItemClick += delegate
                {
                    OnRefreshFromServer();
                };
            }
            ...
            #region Кнопка "Изменить в построителе"
            if (SecureLibrary.IsAllow(UserRightElement.UIEDITREPORT, SelfLibrary.Id) || Workarea.Access.RightCommon.AdminEnterprize)
            {
                BarButtonItem btnEditFromServer = new BarButtonItem
                {
                    Caption = Workarea.Cashe.ResourceString(ResourceString.BTN_CAPTION_EDITRB, 1049),
                    //"Изменить в построителе", //
                    RibbonStyle = RibbonItemStyles.SmallWithText,
                    Glyph = ResourceImage.GetByCode(Workarea, ResourceImage.DOCUMENTNEWEDIT_X16)
                };
                btnEditFromServer.SuperTip = UIHelper.CreateSuperToolTip(btnEditFromServer.Glyph, Workarea.Cashe.ResourceString(ResourceString.BTN_CAPTION_EDITRB, 1049),
                    "Изменить в построителе");

                groupLinksActionTreeList.ItemLinks.Insert(groupLinksActionTreeList.ItemLinks[1], btnEditFromServer);
                btnEditFromServer.ItemClick += delegate
                {
                    OnEditFromServer();
                };
            }
            ...
            #region Список серверов построения отчетов
            BarStaticItem barStaticItem1 = new BarStaticItem {Caption = Workarea.Cashe.ResourceString(ResourceString.STR_REPORTSERVER, 1049)};
            groupLinksActionTreeList.ItemLinks.Insert(groupLinksActionTreeList.ItemLinks[0], barStaticItem1);
            ...
            BarEditItem searchContainer = new BarEditItem();
            ...
            groupLinksActionTreeList.ItemLinks.Insert(groupLinksActionTreeList.ItemLinks[1],searchContainer);
            ...
            //if (collAnalitic.Count > 0)
            //    cmbReportserver
            //
            //cmbReportserver.Items
            #endregion
