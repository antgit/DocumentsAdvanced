...
{
    /// <summary>
    /// Документ "Анкета клиента" в разделе "Маркетинг"
    /// </summary>
    public sealed class DocumentViewMktgAnketa : BaseDocumentViewMktg<DocumentMktg>, IDocumentView
    {
        private BindingSource BindSourceAnaliticDetailsEqupmentPlace;
        private BindingSource BindSourceAnaliticDetailsReasonTrade;
        private BindingSource BindSourceAnaliticDetailsInterest;
        /// <summary>
        ...
        private BindingSource bindAnaliticEqupmentPlace;
        private BindingSource bindAnaliticInterest;
        private BindingSource bindAnaliticReasonTrade;
        ...
        public override void BuildPageCommon()
        {
            if (ControlMain == null)
            {
                if (ControlMain != null)
                    return;
                ControlMain = new ControlMktgCompany
                                  {
                ...
                BindSourceDetails.AddingNew += (sender, eNew) =>
                                                   {
                                                       if (eNew.NewObject == null)
                                                           eNew.NewObject = new DocumentDetailMktg { Workarea = Workarea, Document = SourceDocument, StateId = State.STATEACTIVE };
                                                   };
                ...
                ControlMain.ViewDetail.CellValueChanged += delegate(object sender, DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs eNew)
                                                               {
                                                                   if (eNew.Column.Name == "gridColumnName" || eNew.Column.Name == "gridColumnNom")
                                                                       if (eNew.Value != null && eNew.Value is int)
                                                                       {
                                                                           ...
                                                                           decimal qty = (ControlMain.ViewDetail.GetRow(eNew.RowHandle) as DocumentDetailMktg).Qty;
                                                                           decimal summ = newPrice * qty;
                                                                           decimal discount = (ControlMain.ViewDetail.GetRow(eNew.RowHandle) as DocumentDetailMktg).Discount;
                                                                           ControlMain.ViewDetail.SetRowCellValue(eNew.RowHandle, "Price", newPrice);
                                                                           (ControlMain.ViewDetail.GetRow(eNew.RowHandle) as DocumentDetailMktg).Summa = summ;
                                                                       }
                                                               };
                ControlMain.ViewDetail.RefreshData();

                #region Расположение оборудования
                BindSourceAnaliticDetailsEqupmentPlace = new BindingSource { DataSource = SourceDocument.Analitics };
                ControlMain.GridEqupmentPlace.DataSource = BindSourceAnaliticDetailsEqupmentPlace;
                DataGridViewHelper.GenerateGridColumns(Workarea, ControlMain.editEqupmentPlace.View, "DEFAULT_LOOKUP_ANALITIC");
                
                List<Analitic> collAnaliticEqupmentPlace;
                if (MainDocument.IsReadOnly)
                {
                    IEnumerable<int> equpmentId = SourceDocument.Analitics.Select(f => f.Id);
                    IEnumerable<int> values = equpmentId.Where(f => !Workarea.Cashe.GetCasheData<Analitic>().Exists(f));
                    if (values.Count<int>() == 0)
                    {
                        collAnaliticEqupmentPlace = equpmentId.Select(i => Workarea.Cashe.GetCasheData<Analitic>().Item(i)).ToList();
                    }
                    else
                    {

                        collAnaliticEqupmentPlace = Workarea.GetCollection<Analitic>();
                    }
                }
                else
                    collAnaliticEqupmentPlace = Workarea.GetCollection<Analitic>().Where(s => s.KindValue == Analitic.KINDVALUE_EQUIPMENTPLACEMENT).ToList();
                bindAnaliticEqupmentPlace = new BindingSource { DataSource = collAnaliticEqupmentPlace };

                ControlMain.editEqupmentPlace.DataSource = bindAnaliticEqupmentPlace;
                BindSourceAnaliticDetailsEqupmentPlace.AddingNew += (sender, eNew) =>
                {
                    if (eNew.NewObject == null)
                        eNew.NewObject = new DocumentAnalitic { Workarea = Workarea, Document = SourceDocument.Document, StateId = State.STATEACTIVE };
                };
                ControlMain.ViewEqupmentPlace.CustomRowFilter += ViewDetailCustomRowFilterEqupmentPlace;
                ControlMain.ViewEqupmentPlace.KeyDown += ViewAnaliticEqupmentPlaceKeyDown;
                ControlMain.editEqupmentPlace.ProcessNewValue += AnaliticEqupmentPlaceNomProcessNewValue;
                ControlMain.editEqupmentPlace.View.CustomUnboundColumnData += ViewCustomUnboundColumnDataEqupmentLocation;

                ControlMain.ViewEqupmentPlace.RefreshData(); 
                #endregion

                #region Состояние отношений
                BindSourceAnaliticDetailsReasonTrade = new BindingSource { DataSource = SourceDocument.Analitics };
                ControlMain.GridReasonTrade.DataSource = BindSourceAnaliticDetailsReasonTrade;
                DataGridViewHelper.GenerateGridColumns(Workarea, ControlMain.editReasonTrade.View, "DEFAULT_LOOKUP_ANALITIC");

                List<Analitic> collAnaliticReasonTrade;
                if (MainDocument.IsReadOnly)
                {
                    IEnumerable<int> reasonTradeId = SourceDocument.Analitics.Select(f => f.Id);
                    IEnumerable<int> values = reasonTradeId.Where(f => !Workarea.Cashe.GetCasheData<Analitic>().Exists(f));
                    if (values.Count<int>() == 0)
                    {
                        collAnaliticReasonTrade = reasonTradeId.Select(i => Workarea.Cashe.GetCasheData<Analitic>().Item(i)).ToList();
                    }
                    else
                    {

                        collAnaliticReasonTrade = Workarea.GetCollection<Analitic>();
                    }
                }
                else
                    collAnaliticReasonTrade = Workarea.GetCollection<Analitic>().Where(s => s.KindValue == Analitic.KINDVALUE_REASONTRADEPOINT).ToList();
                bindAnaliticReasonTrade = new BindingSource { DataSource = collAnaliticReasonTrade };

                ControlMain.editReasonTrade.DataSource = bindAnaliticReasonTrade;
                BindSourceAnaliticDetailsReasonTrade.AddingNew += (sender, eNew) =>
                {
                    if (eNew.NewObject == null)
                        eNew.NewObject = new DocumentAnalitic { Workarea = Workarea, Document = SourceDocument.Document, StateId = State.STATEACTIVE };
                };
                ControlMain.ViewReasonTrade.CustomRowFilter += ViewDetailCustomRowFilterReasonTrade;
                ControlMain.ViewReasonTrade.KeyDown += ViewAnaliticReasonTradeKeyDown;
                ControlMain.editReasonTrade.ProcessNewValue += AnaliticReasonTradeProcessNewValue;
                ControlMain.editReasonTrade.View.CustomUnboundColumnData += ViewCustomUnboundColumnDataReasonTrade;

                ControlMain.ViewReasonTrade.RefreshData();
                #endregion

                #region Интересы сотрудничества
                BindSourceAnaliticDetailsInterest = new BindingSource { DataSource = SourceDocument.Analitics };
                ControlMain.GridInterest.DataSource = BindSourceAnaliticDetailsInterest;
                DataGridViewHelper.GenerateGridColumns(Workarea, ControlMain.editInterest.View, "DEFAULT_LOOKUP_ANALITIC");
                ...
                if (MainDocument.IsReadOnly)
                {
                    ...
                    collAnaliticInterest = Workarea.GetCollection<Analitic>().Where(s => s.KindValue == Analitic.KINDVALUE_PRODUCTTYPE).ToList();
                bindAnaliticInterest = new BindingSource { DataSource = collAnaliticInterest };

                ControlMain.editInterest.DataSource = bindAnaliticInterest;
                BindSourceAnaliticDetailsInterest.AddingNew += (sender, eNew) =>
                {
                    if (eNew.NewObject == null)
                        eNew.NewObject = new DocumentAnalitic { Workarea = Workarea, Document = SourceDocument.Document, StateId = State.STATEACTIVE };
                };
                ControlMain.ViewInterest.CustomRowFilter += ViewDetailCustomRowFilterInterest;
                ControlMain.ViewInterest.KeyDown += ViewAnaliticInterestKeyDown;
                ControlMain.editInterest.ProcessNewValue += AnaliticInterestProcessNewValue;
                ControlMain.editInterest.View.CustomUnboundColumnData += ViewCustomUnboundColumnDataInterest;

                ControlMain.ViewInterest.RefreshData();
                #endregion
...
{ 
    ...
    var callingMethod = new System.Diagnostics.StackTrace(1, false) 
        ...
        {
            if ((BindSourceDetails.List[e.ListSourceRow] as DocumentDetailMktg).StateId == State.STATEDELETED)
            {
                e.Visible = false;
                e.Handled = true;
            }
        ...
        // Дополнительный фильтр табличной части документа
        void ViewDetailCustomRowFilterReasonTrade(object sender, DevExpress.XtraGrid.Views.Base.RowFilterEventArgs e)
        {
            if ((BindSourceAnaliticDetailsReasonTrade.List[e.ListSourceRow] as DocumentAnalitic).StateId == State.STATEDELETED)
            {
                e.Visible = false;
                e.Handled = true;
            }
            if ((BindSourceAnaliticDetailsReasonTrade.List[e.ListSourceRow] as DocumentAnalitic).Analitic.KindValue != Analitic.KINDVALUE_REASONTRADEPOINT)
            {
                e.Visible = false;
                e.Handled = true;
            }
        ...
        {
            if ((BindSourceAnaliticDetailsInterest.List[e.ListSourceRow] as DocumentAnalitic).StateId == State.STATEDELETED)
            {
                e.Visible = false;
                e.Handled = true;
            }
            if ((BindSourceAnaliticDetailsInterest.List[e.ListSourceRow] as DocumentAnalitic).Analitic.KindValue != Analitic.KINDVALUE_PRODUCTTYPE)
            {
                e.Visible = false;
                e.Handled = true;
            }
        ...
        // Дополнительный фильтр табличной части документа
        void ViewDetailCustomRowFilterEqupmentPlace(object sender, DevExpress.XtraGrid.Views.Base.RowFilterEventArgs e)
        {
            if ((BindSourceAnaliticDetailsEqupmentPlace.List[e.ListSourceRow] as DocumentAnalitic).StateId == State.STATEDELETED )
            {
                e.Visible = false;
                e.Handled = true;
            }
            if ((BindSourceAnaliticDetailsEqupmentPlace.List[e.ListSourceRow] as DocumentAnalitic).Analitic.KindValue != Analitic.KINDVALUE_EQUIPMENTPLACEMENT)
            {
                e.Visible = false;
                e.Handled = true;
            }
        ...
        void AnaliticReasonTradeProcessNewValue(object sender, ProcessNewValueEventArgs eNv)
        {
            ...
            {
                int index = ControlMain.ViewReasonTrade.FocusedRowHandle;
                DocumentAnalitic docRow = ControlMain.ViewReasonTrade.GetRow(index) as DocumentAnalitic;
                if (docRow != null && docRow.Id == 0)
                {
                    ControlMain.ViewReasonTrade.DeleteRow(index);
                }
            }
        }
        // Обработка изменения в колонке "Номенклатура" табличной части документа 
        void AnaliticInterestProcessNewValue(object sender, ProcessNewValueEventArgs eNv)
        {
            RepositoryItemGridLookUpEdit edit = ((GridLookUpEdit)sender).Properties;

            if (eNv.DisplayValue == null || edit.NullText.Equals(eNv.DisplayValue) || string.Empty.Equals(eNv.DisplayValue))
            {
                int index = ControlMain.ViewInterest.FocusedRowHandle;
                DocumentAnalitic docRow = ControlMain.ViewInterest.GetRow(index) as DocumentAnalitic;
                if (docRow != null && docRow.Id == 0)
                {
                    ControlMain.ViewInterest.DeleteRow(index);
                }
            }
        }
        
        // Обработка изменения в колонке "Номенклатура" табличной части документа 
        void AnaliticEqupmentPlaceNomProcessNewValue(object sender, ProcessNewValueEventArgs eNv)
        {
            RepositoryItemGridLookUpEdit edit = ((GridLookUpEdit)sender).Properties;

            if (eNv.DisplayValue == null || edit.NullText.Equals(eNv.DisplayValue) || string.Empty.Equals(eNv.DisplayValue))
            {
                int index = ControlMain.ViewEqupmentPlace.FocusedRowHandle;
                DocumentAnalitic docRow = ControlMain.ViewEqupmentPlace.GetRow(index) as DocumentAnalitic;
                if (docRow != null && docRow.Id == 0)
                {
                    ControlMain.ViewEqupmentPlace.DeleteRow(index);
                }
            }
        }
        // Обработка изменения в колонке "Номенклатура" табличной части документа 
        void EditNomProcessNewValue(object sender, ProcessNewValueEventArgs eNv)
        {
            ...
            {
                int index = ControlMain.ViewDetail.FocusedRowHandle;
                DocumentDetailMktg docRow = ControlMain.ViewDetail.GetRow(index) as DocumentDetailMktg;
                if (docRow != null && docRow.Id == 0)
                {
            ...
            {
                int index = ControlMain.ViewDetail.FocusedRowHandle;
                if ((ControlMain.ViewDetail.GetRow(index) as DocumentDetailMktg).Product != null)
                    (ControlMain.ViewDetail.GetRow(index) as DocumentDetailMktg).Unit =
                        (ControlMain.ViewDetail.GetRow(index) as DocumentDetailMktg).Product.Unit;
            }
        }
        ...
        // Обработка горячих клавиш в табличной части документа
        void ViewAnaliticInterestKeyDown(object sender, KeyEventArgs eKey)
        {
            if (eKey.KeyCode == Keys.Delete &&
                (SourceDocument.Document.FlagsValue & FlagValue.FLAGREADONLY) != FlagValue.FLAGREADONLY)
            {
                ...
                {
                    int index = ControlMain.ViewInterest.FocusedRowHandle;
                    IDocumentDetail docRow = ControlMain.ViewInterest.GetRow(index) as IDocumentDetail;
                    if (docRow != null)
                    {
                        if (docRow.Id == 0)
                        {
        ...
        // Обработка горячих клавиш в табличной части документа
        void ViewAnaliticReasonTradeKeyDown(object sender, KeyEventArgs eKey)
        {
            if (eKey.KeyCode == Keys.Delete &&
                (SourceDocument.Document.FlagsValue & FlagValue.FLAGREADONLY) != FlagValue.FLAGREADONLY)
            {
                ...
                {
                    int index = ControlMain.ViewReasonTrade.FocusedRowHandle;
                    IDocumentDetail docRow = ControlMain.ViewReasonTrade.GetRow(index) as IDocumentDetail;
                    if (docRow != null)
                    {
                        if (docRow.Id == 0)
                        {
        ...
        // Обработка горячих клавиш в табличной части документа
        void ViewAnaliticEqupmentPlaceKeyDown(object sender, KeyEventArgs eKey)
        {
            if (eKey.KeyCode == Keys.Delete &&
                (SourceDocument.Document.FlagsValue & FlagValue.FLAGREADONLY) != FlagValue.FLAGREADONLY)
            {
                ...
                {
                    int index = ControlMain.ViewEqupmentPlace.FocusedRowHandle;
                    IDocumentDetail docRow = ControlMain.ViewEqupmentPlace.GetRow(index) as IDocumentDetail;
                    if (docRow != null)
                    {
                        if (docRow.Id == 0)
                        {
            ...
            try
            {
                ...
                if (cmb.Name == "cmbAgentFrom" && BindSourceAgentFrom.Count < 2)
                {
                    ...
                    if (rootMyCompany != null)
                        CollectionAgentFrom = rootMyCompany.GetTypeContents<Agent>().Where(f => f.IsMyCompany && !f.IsHiden && f.IsStateAllow).ToList();
                    else
        ...
        void ViewCustomUnboundColumnDataEqupmentLocation(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            DisplayAnaliticImagesLookupGrid(e, bindAnaliticEqupmentPlace);
        }
        void ViewCustomUnboundColumnDataInterest(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            DisplayAnaliticImagesLookupGrid(e, bindAnaliticInterest);
        }
        void ViewCustomUnboundColumnDataReasonTrade(object sender, DevExpress.XtraGrid.Views.Base.CustomColumnDataEventArgs e)
        {
            DisplayAnaliticImagesLookupGrid(e, bindAnaliticReasonTrade);
        }
