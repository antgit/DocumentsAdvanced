...
{
    public abstract class BaseDocumentViewStore<T>:BaseDocumentView<T> where T : class, IDocument, new()
    {
        /// <summary>
        /// Коллекция корреспондентов "Склад Кому"
        /// </summary>
        protected List<Agent> CollectionStoreFrom;

        /// <summary>
        /// Коллекция корреспондентов "Склад Кто"
        /// </summary>
        protected List<Agent> CollectionStoreTo;

        /// <summary>
        /// Источники для связывания корреспондентов "Склад Кому"
        /// </summary>
        public BindingSource BindSourceStoreTo;

        /// <summary>
        /// Источники для связывания корреспондентов "Склад Кто"
        /// </summary>
        public BindingSource BindSourceStoreFrom;

        // элемент отображения
        internal ControlStore ControlMain;
        ...
        protected void RefreshEditEnabled()
        {
            ...
            if (SourceDocument.Document.AgentDepartmentFromId == (SourceDocument as DocumentStore).StoreFromId)
                ControlMain.cmbAgentStoreFrom.Enabled = false;
            if (SourceDocument.Document.AgentDepartmentToId == (SourceDocument as DocumentStore).StoreToId)
