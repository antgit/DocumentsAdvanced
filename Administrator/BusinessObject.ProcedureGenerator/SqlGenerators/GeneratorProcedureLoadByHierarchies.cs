...
{
    internal class GeneratorProcedureLoadByHierarchies : DataObjectGenerator, IDataObjectGenerator2
    {
        public GeneratorProcedureLoadByHierarchies()
        {
            /*
            ...
            Template = "SET ANSI_NULLS ON \n"
                       ...
                       +
                       "	FROM Hierarchy.Hierarchies a INNER JOIN Hierarchy.HierarchyContents b ON a.id = b.HierarchyId \n"
                       + "		 INNER JOIN %schema%.%table% c ON b.ElementId = c.id WHERE a.id = @id \n"
                       + "RETURN 0; \n"
                       + "END TRY \n"
                       + "BEGIN CATCH \n"
        ...
        public override string Generate()
        {
            ...
            string procname = "[Hierarchy].[" + TableName + "LoadByHierarchies" + "]";
        ...
        private void GenerateCustomParamString()
        {
            ...
            foreach (DbObjectChild column in Enumerable.Where<DbObjectChild>(_columns, s => s.IsFormula == false))
            {
                ...
                selectcolumnlist += "c."+column.Name;
