...
{
    internal struct BaseStructDocumentDetailFinance
    {
    ...
    /// </summary>
    public class DocumentDetailFinance : DocumentBaseDetail, IEditableObject,
        IChainsAdvancedList<DocumentDetailFinance, FileData>
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        public DocumentDetailFinance()
            : base()
        {
            _entityId = 6;
        ...
        /// Документ
        /// </summary>
        public DocumentFinance Document { get; set; }
        ...
        #region Состояния
        BaseStructDocumentDetailFinance _baseStruct;
        ...
        public override bool SaveState(bool overwrite)
        {
            ...
            {
                _baseStruct = new BaseStructDocumentDetailFinance
                {
                    Kind = Kind,
                    Memo = _memo,
                    Summa = _summa,
        ...
        public override void Load(SqlDataReader reader, bool endInit = true)
        {
            ...
            try
            {
                ...
                _mGuid = reader.IsDBNull(16) ? Guid.Empty : reader.GetGuid(16);
            }
            catch(Exception ex)
            {
        ...
        internal class TpvCollection : List<DocumentDetailFinance>, IEnumerable<SqlDataRecord>
        {
            IEnumerator<SqlDataRecord> IEnumerable<SqlDataRecord>.GetEnumerator()
            {
                ...
                foreach (DocumentDetailFinance doc in this)
                {
        ...
        internal static IDataRecord FillDataRecort(SqlDataRecord sdr, DocumentDetailFinance doc)
        {
        ...
        #region IChainsAdvancedList<DocumentDetailFinance,FileData> Members

        List<IChainAdvanced<DocumentDetailFinance, FileData>> IChainsAdvancedList<DocumentDetailFinance, FileData>.GetLinks()
        {
            return ((IChainsAdvancedList<DocumentDetailFinance, FileData>)this).GetLinks(55);
        }

        List<IChainAdvanced<DocumentDetailFinance, FileData>> IChainsAdvancedList<DocumentDetailFinance, FileData>.GetLinks(int? kind)
        {
            return GetLinkedFiles();
        }
        List<ChainValueView> IChainsAdvancedList<DocumentDetailFinance, FileData>.GetChainView()
        {
            // TODO: 
            return null; //ChainValueView.GetView<Agent, FileData>(this);
        }
        public List<IChainAdvanced<DocumentDetailFinance, FileData>> GetLinkedFiles()
        {
            List<IChainAdvanced<DocumentDetailFinance, FileData>> collection = new List<IChainAdvanced<DocumentDetailFinance, FileData>>();
                ...
                using (SqlCommand cmd = cnn.CreateCommand())
                {
                    ...
                    try
                    {
                        ...
                        if (reader.HasRows)
                        {
                            while (reader.Read())
                            {
                                ChainAdvanced<DocumentDetailFinance, FileData> item = new ChainAdvanced<DocumentDetailFinance, FileData> { Workarea = Workarea, Left = this };
                                item.Load(reader);
                                collection.Add(item);
                            }
                        }
