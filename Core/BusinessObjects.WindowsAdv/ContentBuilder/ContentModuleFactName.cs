...
{
    /// <summary>
    /// Интерфейсный модуль "Наименования фактов"
    /// </summary>
    public sealed class ContentModuleFactName : ContentModuleBase<FactName>
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        ...
        /// отображения модуля.
        /// Заголовок модуля по умолчанию - "Наименования фактов".
        ...
        /// </remarks>
        public ContentModuleFactName()
        {
            ActiveView = "LIST";
            Caption = "Наименования фактов";
        ...
        void OnShowProp(FactName value)
        {
            value.ShowProperty();
            if (value.IsNew)
            {
        ...
        /// <remarks>После отображения свойств объекта он добавляется в текущий список.</remarks>
        /// <param name="value">Объект для отображения</param>
        void OnShowPropTreeList(FactName value)
        {
            value.ShowProperty();
            if (value.IsNew)
            {
        ...
        void OnSaveObject(FactName value)
        {
            value.Save();
        }
        /// <summary>
        ...
        void ContentModuleShow(object sender, EventArgs e)
        {
            if (BrowserBaseObjects != null)
            {
                if (_showProp == null)
                {
                    _showProp = new Action<FactName>(OnShowProp);
            ...
            if (TreeListBrowser != null)
            {
                ...
                {
                    _showPropTreeList = new Action<FactName>(OnShowPropTreeList);
                    TreeListBrowser.ListBrowserBaseObjects.ShowProperty += _showPropTreeList;
                }
                if (_saveFactName == null)
                {
                    _saveFactName = new Action<FactName>(OnSaveObject);
                    TreeListBrowser.ListBrowserBaseObjects.Save += _saveFactName;
                }
            }
        }
    }
