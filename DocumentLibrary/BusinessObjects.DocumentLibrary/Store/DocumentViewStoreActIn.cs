...
{
    /// <summary>
    /// Документ "Акт прихода товара" в разделе "Управление товарными запасами"
    /// </summary>
    public sealed class DocumentViewStoreActIn : BaseDocumentViewStore<DocumentStore>, IDocumentView
    {
        public void SetEditorValues(object agFrom, object agDepFrom, object agentTo, object agDepTo, object date)
        {
            ...
            ControlMain.cmbAgentDepatmentTo.EditValue = agDepTo;
        }

        #region Создание копии и нового документа
        protected override void CreateCopy()
        {
            DocumentViewStoreActIn newDoc = new DocumentViewStoreActIn();
            newDoc.Showing += (sender, e) =>
                                  {
                                      ...
                                      foreach (DocumentDetailStore item in from prodItem in SourceDocument.Details where prodItem.StateId == State.STATEACTIVE select new DocumentDetailStore { Workarea = Workarea, Price = prodItem.Price, Qty = prodItem.Qty, Summa = prodItem.Summa, Product = prodItem.Product, Unit = prodItem.Unit, Document = newDoc.SourceDocument })
        ...
        //internal void CreateMove(int templateId = 98331)
        //{
        //    if (InvokeSave())
        //    {
        //        DocumentViewStoreMove newDoc = new DocumentViewStoreMove();
        //        newDoc.Showing += delegate
        //        {
        //            int currentAgentFromId = (int)ControlMain.cmbAgentTo.EditValue;
        //            int currentAgentToId = (int)ControlMain.cmbAgentTo.EditValue;
        //            int currentAgentDepatmentToId = (int)ControlMain.cmbAgentDepatmentTo.EditValue;
        //            int currentAgentDepatmentFromId = (int)ControlMain.cmbAgentDepatmentTo.EditValue;
        ...
        //internal void CreateReturnSupplyer(int templateId = 98338)
        //{
        //    if (InvokeSave())
        //    {
        //        DocumentViewStoreReturnSupplyer newDoc = new DocumentViewStoreReturnSupplyer();
        //        newDoc.Showing += delegate
        //        {
        //            int currentAgentFromId = (int)ControlMain.cmbAgentTo.EditValue;
        //            //int currentAgentToId = (int)_ctl.cmbAgentTo.EditValue;
        //            //int currentAgentDepatmentToId = (int)_ctl.cmbAgentDepatmentTo.EditValue;
        //            int currentAgentDepatmentFromId = (int)ControlMain.cmbAgentDepatmentTo.EditValue;
        ...
        //            //}
        //            newDoc.BuildPageCommon();
        //            newDoc.SetEditorValues(currentAgentFromId, currentAgentDepatmentFromId, /*currentAgentToId*/0, /*currentAgentDepatmentToId*/0, ControlMain.dtDate.EditValue);
        ...
        public override void BuildPageCommon()
        {
            if (ControlMain == null)
            {
                ...
                ControlMain.layoutControlItemAgentStoreFrom.Text = Workarea.Cashe.ResourceString(ResourceString.CAPTION_DOC_AGSTORE, 1049);
                ...
                #region Данные для списка "Корреспондент Кто"- Предприятие поставщик
                ...
                #region Данные для списка "Корреспондент Кому" - Предприятие получатель (Наше предприятие)
                ...
                BindSourceDetails.AddingNew += (sender, eNew) =>
                                                   {
                                                       if (eNew.NewObject == null)
                                                           eNew.NewObject = new DocumentDetailStore
                                                                                {
                                                                                    Workarea = Workarea,
        ...
        //{
        //    DevExpress.XtraNavBar.NavBarItem navBarItemCreateMove = new DevExpress.XtraNavBar.NavBarItem
        //                                                                {
        //                                                                    Caption = "Создать перемещение...",
        //                                                                    SmallImage = ExtentionsImage.GetImageDocument(Workarea, State.STATEACTIVE)
        //                                                                };
        //    navBarItemCreateMove.LinkClicked += delegate
        //    {
        //        CreateMove();
        //    };
            ...
            if (ControlMain.ViewDetail.FocusedColumn.Name == "gridColumnName" | ControlMain.ViewDetail.FocusedColumn.Name == "gridColumnNom")
            {
                    ...
                    eEd.Value != null)
                {
                    ...
                    if (prod != null)
                    {
                        ...
                        if ((ControlMain.ViewDetail.GetRow(index) as DocumentDetailStore).Price == 0)
                            (ControlMain.ViewDetail.GetRow(index) as DocumentDetailStore).Price = StoreExtention.GetLastPriceIn(Workarea, ControlMain.dtDate.DateTime, id, (int)ControlMain.cmbAgentDepatmentFrom.EditValue);
                    }
                }
            }
