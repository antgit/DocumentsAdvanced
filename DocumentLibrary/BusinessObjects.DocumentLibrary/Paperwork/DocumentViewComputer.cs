...
{
    /// <summary>
    /// Документ "Учет компьютеров" в разделе "Управление договорами"
    /// </summary>
    public class DocumentViewComputer : BaseDocumentView<DocumentContract>, IDocumentView //BaseDocumentViewContract<DocumentContract>, IDocumentView
    {
        #region Базовые функции
                // элемент отображения
        internal ControlContractComputer ControlMain;
        ...
        {
            DocumentViewComputer newDoc = new DocumentViewComputer();
        ...
        public override void BuildPageCommon()
        {
            if (ControlMain == null)
            {
                if (ControlMain != null)
                    return;
                ControlMain = new ControlContractComputer { Name = ExtentionString.CONTROL_COMMON_NAME };
                ...
                ControlMain.editEqupmentState.DataSource = bindEqupmentState;
                ...
                if (MainDocument.IsReadOnly)
                {
                    ...
                    else
                    {
                        ...
                        collProducts = Workarea.GetCollection<Product>(Product.KINDVALUE_COMPUTER);
                    }
                }
                else
                    collProducts = Workarea.GetCollection<Product>(Product.KINDVALUE_COMPUTER);
                ...
                foreach (DocumentDetailContract row in SourceDocument.Details)
                {
                    if (row.ProductId != 0)
                    {
                        row.StringValue1 = row.Product.Memo;
                    }
                }
            }
        ...
        public override bool InvokeSave()
        {
            ...
            try
            {
                ...
                // сохранение данных о конфигурации компьютеров
                foreach (DocumentDetailContract row in SourceDocument.Details.Where(f=>f.StateId!=State.STATEDELETED))
                {
                    if(row.ProductId!=0)
                    {
                        row.Product.Memo = row.StringValue1;
                        row.Product.Save();
                        row.StringValue1 = string.Empty;
                    }
                }
