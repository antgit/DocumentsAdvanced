...
namespace BusinessObjects.Workflows
{
    ...
    public class WorkflowManager
    {
        ...
        public static WorkflowManager GetWorkflowManagerWa(Workarea wa)
        {
            WorkflowManager mgr = new WorkflowManager();
            mgr.InitializeRuntime();
            //string connectionString = connectionString; //@"Data Source=ineroth;Initial Catalog=WorkflowPersistenceStore; Persist Security Info=True;User ID=test;Password=test";
            ...
            //new SqlWorkflowPersistenceService(wa.GetDatabaseConnectionAdmin().ConnectionString, unloadOnIdle,
            //instanceOwnershipDuration, loadingInterval);
            //mgr.RuntimeInstance.AddService(persistService);
            
            
            //WorkflowApplication wfApp = new WorkflowApplication(new Workflow1(), inputs);
        ...
        public IDictionary<string, object> ExecuteWorkflow(Activity activity, Dictionary<string, object> parameters)
        {
            return WorkflowInvoker.Invoke(activity, parameters);
        }
        /// <summary>
        /// Synchronously executes a workflow.
        /// </summary>
