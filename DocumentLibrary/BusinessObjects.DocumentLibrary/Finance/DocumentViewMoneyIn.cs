...
namespace BusinessObjects.DocumentLibrary.Finance
{
    ...
    /// </summary>
    public sealed class DocumentViewMoneyIn : BaseDocumentViewFinance<DocumentFinance>, IDocumentView
    {
        #region Создание копии и нового документа
        protected override void CreateCopy()
        {
            DocumentViewMoneyIn newDoc = new DocumentViewMoneyIn();
        ...
        public override void BuildPageCommon()
        {
            if (ControlMain == null)
            {
                ...
                #region Данные для списка "Тип платежа"
...
//            prnDoc.SummaTotal = Summa + prnDoc.SummaNds;
            ...
            try
            {
                ...
                PreparePrintFinance doc = new PreparePrintFinance { SourceDocument = SourceDocument };
        ...
        void CmbAgentGridLookUpEditQueryPopUp(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    Hierarchy rootPaymentType = Workarea.Cashe.GetCasheData<Hierarchy>().ItemCode<Hierarchy>(Hierarchy.SYSTEM_ANALITIC_PAYMENTTYPEIN);
