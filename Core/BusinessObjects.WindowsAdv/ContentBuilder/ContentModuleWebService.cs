...
{
    /// <summary>
    /// Интерфейсный модуль "Службы"
    /// </summary>
    public sealed class ContentModuleWebService : ContentModuleBase<WebService>
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        ...
        /// отображения модуля.
        /// Заголовок модуля по умолчанию - "Службы".
        ...
        /// </remarks>
        public ContentModuleWebService()
        {
            Caption = "Службы";
        ...
        {
            Image32 = ResourceImage.GetByCode(Workarea, ResourceImage.CLIENTTOSERVER_X32);
        }
        /// <summary>
        /// Делегат отображения свойств объекта при отображении в виде списка
        /// </summary>
        Action<WebService> _showProp;
        /// <summary>
        /// Делегат отображения свойств объекта при отображении в виде групп
        /// </summary>
        Action<WebService> _showPropTreeList;
        /// <summary>
        /// Делегат сохранения объекта
        /// </summary>
        Action<WebService> _saveAnalitic;
        /// <summary>
        ...
        void OnShowProp(WebService value)
        {
            value.ShowProperty();
            if (value.IsNew)
            {
        ...
        /// <remarks>После отображения свойств объекта он добавляется в текущий список.</remarks>
        /// <param name="value">Объект для отображения</param>
        void OnShowPropTreeList(WebService value)
        {
            value.ShowProperty();
            if (value.IsNew)
            {
        ...
        void OnSaveObject(WebService value)
        {
            value.Save();
        }
        /// <summary>
        ...
        void ContentModuleShow(object sender, EventArgs e)
        {
            if (BrowserBaseObjects != null)
            {
                if (_showProp == null)
                {
                    _showProp = new Action<WebService>(OnShowProp);
            ...
            if (TreeListBrowser != null)
            {
                ...
                {
                    _showPropTreeList = new Action<WebService>(OnShowPropTreeList);
                    TreeListBrowser.ListBrowserBaseObjects.ShowProperty += _showPropTreeList;
                }
                if (_saveAnalitic == null)
                {
                    _saveAnalitic = new Action<WebService>(OnSaveObject);
