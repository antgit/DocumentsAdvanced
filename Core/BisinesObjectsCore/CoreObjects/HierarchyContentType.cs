...
{
    /// <summary>
    /// Привязка типов объектов к иерархиям
    /// </summary>
    public sealed class HierarchyContentType
        : BaseCoreObject
    {
        public HierarchyContentType()
            : base()
        {
            EntityId = (short)WhellKnownDbEntity.HierarchyContentType;
        }
        ...
        /// <returns></returns>
        public static List<HierarchyContentType> GetCollection(Workarea wa, int columnId)
        {
            HierarchyContentType item;
            List<HierarchyContentType> collection = new List<HierarchyContentType>();
            using (SqlConnection cnn = wa.GetDatabaseConnection())
            {
                ...
                try
                {
                    using (SqlCommand cmd = cnn.CreateCommand())
                    {
                        ...
                        cmd.CommandText = wa.Empty<Hierarchy>().Entity.FindMethod("HierarchyContentTypesLoadByElementId").FullName;
                        SqlDataReader reader = cmd.ExecuteReader();
                        while (reader.Read())
                        {
                            item = new HierarchyContentType { Workarea = wa };
                            item.Load(reader);
                            collection.Add(item);
                        }
                        reader.Close();
