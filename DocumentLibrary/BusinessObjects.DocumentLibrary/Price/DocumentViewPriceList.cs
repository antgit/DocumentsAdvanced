...
{
    /// <summary>
    /// Документ "Прайс-лист" в разделе "Ценовая плолитика"
    /// </summary>
    /// <remarks>Документ для учета цен и выполнения расчетов цен.</remarks>
    public sealed class DocumentViewPriceList : BaseDocumentViewPrice<DocumentPrices>, IDocumentView
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        public DocumentViewPriceList()
            : base()
        {
        ...
        protected override void CreateCopy()
        {
            ...
            newDoc.Showing += delegate
            {
                ...
                foreach (DocumentDetailPrice item in from prodItem in SourceDocument.Details where prodItem.StateId == State.STATEACTIVE select new DocumentDetailPrice
                                                                                                                                                    {
                                                                                                                                                        Workarea = Workarea, 
        ...
        protected override void Print(int id, bool withPrewiew)
        {
            ...
            List<PrintDataDocumentProductDetail> collection = items.Select(item => new PrintDataDocumentProductDetail
                                                                                       {
                                                                                           Price = item.Value, Memo = item.Memo, ProductCode = item.Product.Nomenclature, ProductName = item.Product.Name, UnitName = (item.Product.UnitId != 0 ? item.Product.Unit.Code : string.Empty)
                                                                                       }).ToList();
            #endregion
            try
            {
