...
{
    /// <summary>
    /// Документ "Прайс-лист конкурента индивидуальный" в разделе "Ценовая плолитика"
    /// </summary>
    /// <remarks>Документ для учета цен и выполнения расчетов цен.</remarks>
    public sealed class DocumentViewPriceListCompetitorClient : BaseDocumentViewPrice<DocumentPrices>, IDocumentView
    {
        /// <summary>
        /// Конструктор
        /// </summary>
        public DocumentViewPriceListCompetitorClient()
            : base()
        {
        ...
        {
            DocumentViewPriceListCompetitorClient newDoc = new DocumentViewPriceListCompetitorClient();
        ...
        void CmbAgentGridLookUpEditQueryPopUp(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    CollectionAgentFrom = Workarea.GetCollection<Agent>(Agent.KINDVALUE_COMPANY).Where(s=>!s.IsHiden && s.IsStateAllow).ToList();
                ...
                {
                    CollectionAgentTo = Workarea.GetCollection<Agent>(Agent.KINDVALUE_COMPANY).Where(s=>!s.IsHiden && s.IsStateAllow).ToList();
                ...
                {
                    CollectionPrice = Workarea.GetCollection<PriceName>(PriceName.KINDVALUE_COMPETITOR).Where(s=>!s.IsHiden && s.IsStateAllow).ToList();
